...
{
  return preg_replace_callback(SF_AUTO_LINK_RE, '_sa_url_cmd', $text);
}

function _sa_url_cmd($matches)
{
...
function sa_auto_link_text_for_mobile($text, $link = null, $href_options = array(), $truncate = true, $truncate_len = 37, $pad = '...', $is_allow_outer_url = null)
{
  ...
  {
    $result = _sa_auto_links_phone_number($result);
  }
  if (in_array('urls', $link))
  {
    $result = _sa_auto_links_urls($result, $href_options, $truncate, $truncate_len, $pad);
    if ($is_allow_outer_url)
    {
      $result = _sa_auto_links_outer_urls($result, $href_options, $truncate, $truncate_len, $pad);
    }
  }
...
function _sa_auto_links_urls($text, $href_options = array(), $truncate = false, $truncate_len = 40, $pad = '...')
{
...
function _sa_auto_links_outer_urls($text, $href_options = array(), $truncate = false, $truncate_len = 40, $pad = '...')
{
  $request = sfContext::getInstance()->getRequest();
  $href_options = _tag_options($href_options);
...
function _sa_auto_links_phone_number($text)
{
