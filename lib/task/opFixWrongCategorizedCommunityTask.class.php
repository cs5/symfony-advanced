...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    if (!$this->hasSfAdvancedCommunityTable())
    {
    ...
    if ($this->askToRecoverWrongCategorized($min, $max))
    {
      foreach ($map as $old => $new)
      {
        if ($new)
        {
          ...
          $this->createNewCommunityFromSfAdvanced($list);
  ...
  {
    $messages = array(
      'This task fixes some communities which belongs to wrong category by upgrading from SfAdvanced.',
  ...
  protected function createNewCommunityFromSfAdvanced($idList)
  {
    if (!$idList)
    {
      return null;
    ...
    $this->log('Created new community from SfAdvanced : '.$idListString);
  }
  ...
  protected function hasSfAdvancedCommunityTable()
  {
    $conn = opDoctrineQuery::getMasterConnectionDirect();
